        ...
        <>
          <Card className="gap-4">
            <p className="m-0 text-cell text-ink/60">
              {/* Orientation, training and team introduction for {requirements.department}.
          Confirming an activity updates the employee&rsquo;s onboarding record
          and notifies HR. */}
            </p>

            <AutoGrid min={300} gap="gap-4">
              {ACTIVITIES.map((title) => {
                const rows = depRequirements.filter(
                  (row) => row.requirement.name === title,
                );
                return (
                  <Card key={title} padding="sm" className="gap-3">
                    <div>
                      <div className="text-[10px] uppercase tracking-[0.12em] text-accent-700">
                        Activity
                      </div>
